...
plot(auc_1$perf, col = "black", lty = 3, 
     main = paste0("Health AIC - 5-fold CV AUC"))
...
plot(auc_2$perf, col = "black", lty = 3, 
     main = paste0("Health BIC - 5-fold CV AUC"))
plot(auc_2$perf, col = "blue", lty = 1, avg = "vertical", add = TRUE)
text(0.8, 0.2, paste("mean AUC:", format(round(auc_2$cvAUC, 4), nsmall = 4)))
text(0.8, 0.1, paste("AUC:", format(round(min(auc_2$fold.AUC), 4), nsmall = 4), 
                     "-", format(round(max(auc_2$fold.AUC), 4), nsmall = 4)))

auc_3 <- cvAUC(checkup_aic_preds, checkup_aic_actuals)
plot(auc_3$perf, col = "black", lty = 3, 
     main = paste0("Checkup AIC - 5-fold CV AUC"))
plot(auc_3$perf, col = "blue", lty = 1, avg = "vertical", add = TRUE)
text(0.8,0.2, paste("mean AUC:", format(round(auc_3$cvAUC, 4), nsmall = 4)))
text(0.8,0.1, paste("AUC:", format(round(min(auc_3$fold.AUC), 4), nsmall = 4), 
                    "-", format(round(max(auc_3$fold.AUC), 4), nsmall = 4)))

auc_4 <- cvAUC(checkup_bic_preds, checkup_bic_actuals)
plot(auc_4$perf, col = "black", lty = 3, 
     main = paste0("Checkup BIC - 5-fold CV AUC"))
plot(auc_4$perf, col = "blue", lty = 1, avg = "vertical", add = TRUE)
text(0.8,0.2, paste("mean AUC:", format(round(auc_4$cvAUC, 4), nsmall = 4)))
text(0.8,0.1, paste("AUC:", format(round(min(auc_4$fold.AUC), 4), nsmall = 4),
